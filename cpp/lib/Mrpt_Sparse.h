#ifndef CPP_MRPT_SPARSE_H_
#define CPP_MRPT_SPARSE_H_
...
struct MrptSparse_Parameters {
  ...
  double estimated_qtime =
      0.0; /**< Estimated query time (if the index is autotuned and the target
              recall is set; otherwise 0.0). */
  double estimated_recall =
      0.0; /**< Estimated recall (if the index is autotuned and the target
              recall is set; otherwise 0.0). */
};
...
public:
  ...
  void grow(int n_trees_, int depth_, float density_ = -1.0, int seed = 0) {
    ...
    if (density_ < -1.0001 || density_ > 1.0001 ||
        (density_ > -0.9999 && density_ < -0.0001)) {
    ...
    density < 1
        ? build_sparse_random_matrix(sparse_random_matrix, n_pool, dim, density,
                                     seed)
      ...
      if (sparse_input) {
        if (density < 1) {
          ...
          tree_projections.noalias() =
              dense_random_matrix.middleRows(n_tree * depth, depth) * X_Sparse;
      } else {
        if (density < 1)
          tree_projections.noalias() =
   ...
   * a default value 0 initializes the rng randomly with std::random_device
   */
  void grow(double target_recall, const Eigen::Ref<const Eigen::MatrixXf> &Q,
            int k_, int trees_max = -1, int depth_max = -1, int depth_min_ = -1,
   ...
   */
  void grow(double target_recall, const float *Q, int n_test, int k_,
  ...
  void grow_autotune(double target_recall, int k_, int trees_max = -1,
    ...
    std::cout << " calling autotune n_samples" << n_samples << "n_test "
              << n_test << std::endl;
   ...
   * separate test set, leave empty.
   **/
  void grow(const float *data, int n_test, int k_, int trees_max = -1,
...
//    const Eigen::Map<const Eigen::MatrixXf> Q(data, dim, n_test);
   ...
   * a default value 0 initializes the rng randomly with std::random_device
   */
  void grow(const Eigen::Ref<const Eigen::MatrixXf> &Q, int k_,
            int trees_max = -1, int depth_max = -1, int depth_min_ = -1,
            int votes_max_ = -1, float density_ = -1.0, int seed = 0) {
    if (Q.rows() != dim) {
    ...
    grow(Q.data(), Q.cols(), k_, trees_max, depth_max, depth_min_, votes_max_,
         density_, seed);
  }
   ...
   * @param n_test number of test queries sampled from the training set.
   */
  void grow_autotune(int k_, int trees_max = -1, int depth_max = -1,
                     int depth_min_ = -1, int votes_max_ = -1,
                     float density_ = -1.0, int seed = 0, int n_test = 100) {
    if (n_test < 1) {
      throw std::out_of_range("Test set size must be > 0.");
    }
  ...
  MrptSparse subset(double target_recall, MrptSparse &index2) const{
    ...
    index2.tree_leaves.assign(tree_leaves.begin(),
                              tree_leaves.begin() + index2.n_trees);
    ...
    if (index2.density < 1) {
      ...
      index2.dense_random_matrix =
          Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>(
              index2.n_pool, index2.dim);
      for (int n_tree = 0; n_tree < index2.n_trees; ++n_tree)
        index2.dense_random_matrix.middleRows(n_tree * index2.depth,
                                              index2.depth) =
  ...
  MrptSparse *subset_pointer(double target_recall) const {
    ...
    if (index2->density < 1) {
      index2->sparse_random_matrix =
          Eigen::SparseMatrix<float, Eigen::RowMajor>(index2->n_pool,
                                                      index2->dim);
      for (int n_tree = 0; n_tree < index2->n_trees; ++n_tree)
        index2->sparse_random_matrix.middleRows(n_tree * index2->depth,
                                                index2->depth) =
...
#pragma omp parallel for
   ...
   * set size
   */
  void query(const Eigen::Ref<const Eigen::VectorXf> &q, int k,
             int vote_threshold, int *out, float *out_distances = nullptr,
             int *out_n_elected = nullptr) const {
...
#pragma omp parallel for
   ...
   * k nearest neighbors
   */
  static void exact_knn(const float *q_data, const float *X_data, int dim,
                        int n_samples, int k, int *out,
                        float *out_distances = nullptr) {
...
private:
   ...
   * by the median.
   */
  void grow_subtree(std::vector<int>::iterator begin,
                    std::vector<int>::iterator end, int tree_level, int i,
                    int n_tree, const Eigen::MatrixXf &tree_projections) {
    int n = end - begin;
    int idx_left = 2 * i + 1;
    ...
    if (n % 2) {
      ...
      auto left_it = std::max_element(
          begin, mid, [&tree_projections, tree_level](int i1, int i2) {
            return tree_projections(tree_level, i1) <
                   tree_projections(tree_level, i2);
          });
      split_points(i, n_tree) = (tree_projections(tree_level, *mid) +
                                 tree_projections(tree_level, *left_it)) /
                                2.0;
    }
...
#pragma omp parallel for
  ...
  static void build_sparse_random_matrix(
    ...
    sparse_random_matrix =
        Eigen::SparseMatrix<float, Eigen::RowMajor>(n_row, n_col);
  ...
  static void build_dense_random_matrix(
    ...
    std::generate(dense_random_matrix.data(),
                  dense_random_matrix.data() + n_row * n_col,
  ...
  void vote(const Eigen::VectorXf &projected_query, int vote_threshold,
            Eigen::VectorXi &elected, int &n_elected, int n_trees,
            int depth_crnt) {
    std::vector<int> found_leaves(n_trees);
    const std::vector<int> &leaf_first_indices =
        leaf_first_indices_all[depth_crnt];
...
#pragma omp parallel for
  ...
  fit_projection_times(const Eigen::Map<const Eigen::MatrixXf> &Q,
  ...
  static void generate_x(std::vector<int> &x, int max_generated, int n_tested,
                         int max_val) {
    ...
    auto end = std::remove_if(x.begin(), x.end(),
                              [max_val](int t) { return t > max_val; });
    x.erase(end, x.end());
  }

  std::pair<double, double>
  fit_exact_times(const Eigen::Map<const Eigen::MatrixXf> &Q) {
    ...
    for (int i = 0; i < (int)s_tested.size(); ++i) {
      ...
      for (int m = 0; m < n_sim; ++m) {
        ...
        exact_knn(Eigen::Map<const Eigen::VectorXf>(Q.data() + ri * dim, dim),
                  k, elected, s_size, &res[0]);
  ...
  static std::pair<double, double> fit_theil_sen(const std::vector<double> &x,
    ...
    std::nth_element(slopes.begin(), slopes.begin() + n_slopes / 2,
                     slopes.end());
   ...
   * to the left branch.
   */
  static void count_leaf_sizes(int n, int level, int tree_depth,
                               std::vector<int> &out_leaf_sizes) {
    if (level == tree_depth) {
   ...
   * the left branch.
   */
  static void count_first_leaf_indices(std::vector<int> &indices, int n,
                                       int depth) {
  ...
  static void
  count_first_leaf_indices_all(std::vector<std::vector<int>> &indices, int n,
                               int depth_max) {
  ...
  double get_voting_time(int n_trees, int depth, int v) const {
    const std::map<int, std::pair<double, double>> &beta =
        beta_voting[depth - depth_min];
  ...
  double get_exact_time(int n_trees, int depth, int v) const {
    return predict_theil_sen(get_candidate_set_size(n_trees, depth, v),
                             beta_exact);
  }

  double get_query_time(int tree, int depth, int v) const {
    return get_projection_time(tree, depth, v) +
           get_voting_time(tree, depth, v) + get_exact_time(tree, depth, v);
  }
  ...
  std::vector<int> sample_indices(int n_test, int seed = 0) const {
    ...
    return std::vector<int>(indices_data.begin(),
                            indices_data.begin() + n_test);
  }
  ...
  float density =
      -1.0; // expected ratio of non-zero components in a projection matrix
